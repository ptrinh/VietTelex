...
public struct TelexEngine {
    ...
    /// Live spell-check (OpenKey's `tempDisableKey`). When true, as soon as the word
    ...
    /// Simple Telex (OpenKey's input mode). When true, a STANDALONE `w` (no adjacent
    ...
    /// (aw→ă, ow→ơ, uw→ư). Matches OpenKey issue #223. (Brackets are already literal in
    ...
    private func parse(into letters: inout [LetterUnit], rawLetter: inout [Int],
        ...
        while i < rawCount {
            ...
            if lower == UInt8(ascii: "w") {
                ...
                // hoăc), where breve on a is correct. Makes marks order-free like
                // OpenKey: "nuawx" and "nuwax" both give "nữa".
                if tIdx >= 1,
