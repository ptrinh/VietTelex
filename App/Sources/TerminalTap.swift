...
// raw key.
//
// So for terminals we bypass IMKit entirely and act like OpenKey/EVKey: a CGEventTap
...
/// up in NSWorkspace.frontmostApplication. Detect it the way OpenKey does: scan the
...
enum SyntheticKeyboard {
    ...
    ///   suggestion). Mirrors OpenKey's `SendShiftAndLeftArrow`. A pure deletion
...
/// Global keyDown tap that handles terminal-class apps (OpenKey-style). Owns its own
...
final class TerminalTapController {
    ...
    private func handle(type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        // abandon it (like OpenKey's RequestNewSession). We do NOT emit backspaces/
        ...
        // reset our engine AND signal the controller, mirroring OpenKey's global reset.
