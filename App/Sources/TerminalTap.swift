...
final class TerminalTapController {
    ...
    private func handle(type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        guard let ascii = ch.asciiValue, isLetterAscii(ascii) else {
            ...
            // B1: if emitBoundary rewrote nothing (queue drained), land the boundary
            // char by rewriting THIS event in place; if it did post a rewrite the
            // queueDrained() guard declines, so we fall back to the synthetic re-emit
            // and the boundary char still sequences after the rewrite.
            if modifyInPlace(event: event, backspaces: 0, insert: String(ch)) { return pass }
