...
`Scripts/make-release.sh` tạo CẢ HAI artifact vào `~/Desktop` —
`VietTelex-<VER>.app.zip` (zip từ app đã staple) và `VietTelex-<VER>.pkg` — rồi in
...
đính `.app.zip`. Sau khi `gh release upload`, bump `Casks/viettelex.rb` trong tap
`ptrinh/homebrew-viettelex`: `version` + `sha256` (in ra từ script).
