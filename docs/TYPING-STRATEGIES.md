...
| # | Đường | Kênh | Cần AX? | MAS? | Underline? | Latency (đo được) |
|---|---|---|---|---|---|---|
| 1 | **In-place** | IMKit `insertText(_:replacementRange:)` | Không | ✅ | Không | imk.handle p50 ~1.9 ms (XPC insertText/selectedRange) |
| 2 | **Marked text** | IMKit `setMarkedText` | Không | ✅ | **Có** (khi đang gõ) | Tương đương in-place (cùng kênh IMKit XPC) |
| 3 | **Tap: backspace-retype** | CGEventTap + synthetic events | **Có** | ❌ | Không | tap.handle ~18 µs; e2e burst dấu ~+13 ms so với phím thường (pty đo) |
| 4 | **Tap: selection-replace** | CGEventTap, Shift+←×N rồi ghi đè | **Có** | ❌ | Không | 2(N+1) posted events ~3 ms/event; D1 (AX write) gộp còn 1 call, default OFF |
| 5 | **Tap: empty-reset** | CGEventTap, chèn U+202F rồi backspace-retype | **Có** | ❌ | Không | Như #3 + 1 event chèn/xóa ký tự mồi |
